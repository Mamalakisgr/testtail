<template>
    <div class="flex min-h-screen bg-gray-100 dark:bg-gray-800">
      ...
      <div class="flex-1 p-8">
        <h2 class="text-2xl font-semibold text-gray-800 dark:text-white">Shipping Addresses</h2>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg overflow-hidden shadow-lg dark:bg-gray-800">
          <div class="p-4">
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white">{{ isEditing ? 'Edit Address' : 'Add New Address' }}</h3>
...
const fetchAddresses = async () => {
  try {
    const response = await axios.get(`${backendUrl}/api/user/shipping-address`, { withCredentials: true });
    addressesRaw.value = response.data;
  } catch (error) {
...
const saveAddress = async () => {
  try {
    const response = await axios.post(`${backendUrl}/api/user/shipping-address`, {
    ...
    addressesRaw.value = response.data.shippingAddresses;
...
const deleteAddress = async () => {
  try {
    await axios.delete(`${backendUrl}/api/user/shipping-address/${addressToDeleteId.value}`, { withCredentials: true });
