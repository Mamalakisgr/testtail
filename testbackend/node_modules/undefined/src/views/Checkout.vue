<template>
  <div
    class="container mx-auto p-6 shadow-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-900 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500 bg-gray-700">
    <div class="grid grid-cols-1 lg:grid-cols-12 lg:gap-10 lg:pt-12">
      ...
      <div class="lg:col-span-8 space-y-6">
        ...
        <ShippingForm ref="ShippingForm" v-model:sameAsShipping="sameAsShipping" v-model:saveInfo="saveInfo"
          :formData="formData" @submit="handleFormSubmit" @toggle-shipping-address="toggleShippingAddress" />


        <!-- Conditionally Display Billing Address Form -->
        <div v-if="!sameAsShipping" class="max-w-lg mx-auto bg-gray-700 p-8 rounded-lg">
          <h2 class="text-lg font-semibold mb-4 text-white ">Billing Address</h2>
          <div class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 ">
              <input type="text" v-model="formData.billingFirstName" placeholder="Billing First Name"
                class="p-3 rounded-lg bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:outline-none border  " />
              <input type="text" v-model="formData.billingLastName" placeholder="Billing Last Name"
                class="border p-3 rounded-lg bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:outline-none" />
            </div>
            <input type="email" v-model="formData.billingEmail" placeholder="Billing Email"
              class="border w-full p-3 rounded-lg bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:outline-none" />
            <input type="text" v-model="formData.billingAddress" placeholder="Billing Address"
              class="border w-full p-3 rounded-lg bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:outline-none" />
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <input type="text" v-model="formData.billingCity" placeholder="Billing City"
                class="border p-3 rounded-lg bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:outline-none" />
              <input type="text" v-model="formData.billingZip" placeholder="Billing Zip Code"
                class="border p-3 rounded-lg bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:outline-none" />
            </div>
            <input type="text" v-model="formData.billingCountry" placeholder="Billing Country"
              class="border w-full p-3 rounded-lg bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:outline-none" />
          </div>
        </div>
        ...
        <button @click="finishPayment" type="button"
          class="w-full text-center px-4 py-3 bg-blue-500 rounded-md shadow-md text-white font-semibold">
          Finish payment
        </button>
        <Alert :visible="alertVisible" position="top-right" :color="alertColor" :title="alertTitle"
          :description="alertDescription" />
      </div>
      <div class="lg:col-span-4">
        <div class="sticky top-20">
          <OrderSynopsis :deliveryOption="formData.deliveryMethod" :totalPrice="formData.totalPrice"
            :originalPrice="formData.originalPrice" :savings="formData.savings"
            :deliveryPickup="formData.deliveryPickup" :tax="formData.tax" @update:totalPrice="updateTotalPrice" />
